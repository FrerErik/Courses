...
public class RandomScheduling {
  ...
  public static class ScheduledProcess {
    ...
    public ScheduledProcess(int id, int burstTime, int arrivalTime) {
      this.id = id;
  ...
  List<ScheduledProcess> registry = new ArrayList<>();
  ...
  public void runNewSimulation(final boolean isPreemptive, final int timeQuantum,
    ...
    List<ScheduledProcess> processors = new ArrayList<>();
    ...
    while (simulation) {
      // System.out.println("Cycle: " + tick);
      // chance to add a new process
      id = ArrivalChance(probArrival, processors, id, maxBurstTime, minBurstTime, tick, maxArrivalsPerTick, addProcess,
          numProcesses);
      ...
      if (processors.size() < 1) {
        ...
        for (ScheduledProcess nonprocessing : processors) {
          if (!nonprocessing.equals(processors.get(index))) {
            if (nonprocessing.getBurstTime() == nonprocessing.getProcessingTime()) {

            } else {
              nonprocessing.incrementWaitingTime();
            }
          }
        }
        ...
        // if the processing time is equal to the burst time we will consider the
        // process done and hence added to the registry
    ...
    // each process starts with a burst value between 2 and 10
    ...
    // TODO:
    // 1. Run a simulation as a loop, with one iteration considered as one unit of
    // time (tick)
    // 2. The simulation should involve the provided number of processes
    // "numProcesses"
    // 3. The simulation loop should finish after the all of the processes have
    // completed
    // 4. On each tick, a new process might arrive with the given probability
    // (chance)
    ...
    // should pick one of the available processes *at random* and start its
    // execution
    // 8. If the preemptive version of the scheduling algorithm is invoked, then it
    // should
  ...
  public void printResults() {
    // TODO:
    // 1. For each process, print its ID, burst time, arrival time, and total
    // waiting time
    // 2. Afterwards, print the complete execution time of the simulation
    // and the average process waiting time
    int totalWaitingTime = 0;
    int averageTime = 0;
    for (ScheduledProcess processor : registry) {
      int id = processor.getId();
      int burstTime = processor.getBurstTime();

      int arrivalTime = processor.getArrivalTime();
      totalWaitingTime = (processor.getWaitingTime());

      System.out.print(" id: " + id + "| burstTime: " + burstTime + "| arrivalTime: " + arrivalTime
          + "| waitingTime: " + processor.getWaitingTime() + "| \n");

    }
    System.out.println("--------------------------------");
    System.out.println("Execution time: " + totalSimulationTime + "\n" + "--------------------------------");
    for (ScheduledProcess processor : registry) {
      averageTime += processor.getWaitingTime();
    }
