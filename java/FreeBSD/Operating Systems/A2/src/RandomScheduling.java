...
public class RandomScheduling {
	...
	// ... add further fields and methods, if necessary
	int ticks = 0;
	List<ScheduledProcess> processesRegistry = new ArrayList<>();
	...
	public void reset() {
		ticks = 0;
		processesRegistry = new ArrayList<>();
	}
	...
	public void runNewSimulation(final boolean isPreemptive, final int timeQuantum,
		...
		while(processedCounter<numProcesses) {
			...
			if(rng.nextDouble()<probArrival) {
				...
				for(int i = 1; i <= incomingProcessNum; i++) {
					if(processesRegistry.size()<numProcesses) {
						ScheduledProcess process = new ScheduledProcess(
										id,rng.nextInt(minBurstTime,maxBurstTime+1),ticks);
...
//				System.out.println(index);
        processForRun = scheduledProcessList.get(rng.nextInt(0,scheduledProcessList.size()));
			}
	...
	public void printResults() {
		...
		for(ScheduledProcess p : processesRegistry) {
		...
		System.out.println("Average waiting time is: " + sumWaitingTime / processesRegistry.size());
	}
		
	
	public static void main(String args[]) {
		// TODO: replace the seed value below with your birth date, e.g., "20001001"
		final long rngSeed = 19901004;
