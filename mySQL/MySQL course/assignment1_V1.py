...
# Database name 
DB_NAME = 'Duan_Eriksson' #fe1 fixed my last name
...
while(flag):
    ...
    # Selection 2 answers question 2 
        ...
        ident = [] # fe1 Added a method to find if the planet is in the database or not
        query = (''' SELECT name FROM planets ''')
        cursor.execute(query)
        for name in cursor:
            for i in name:
                ident.append(i)
        if planet_name not in ident:
    ...
    # Selection 3 answers question 3 
        ...
        average_height = int(input("Enter a height for query: "))
        ...
        for j in lst1:
            if j[1] != 'NA': # Filter the species without height information which showing 'NA' in the table
                ...
                if height > average_height:
                    lst2.append(j)
        if not lst2:
            print(f"\nNo species found with height higher than {average_height}.") #fe1 added 
            flag = False
            flag = return_to_menu(flag)
        else:
            print(f"\nThe species with height higher than {average_height}: ")  #fe1 changed "given number" , to the actual number
            for sp in lst2:
                print(">", sp[0] + ":  " + sp[1])
            flag = False
            flag = return_to_menu(flag)
    ...
    # Selection 4 answers question 4
        ...
        ident = [] #fe1 added a way to identify if the specie is in the database.
        query = ('''SELECT sp_name FROM species''')
        cursor.execute(query)
        for sp_name in cursor:
            for i in sp_name:
                ident.append(i)
        if species_name not in ident:
            ...
            for i in cursor:
                for j in i:
                    if j == 'NA': # Identify the species without 'homeworld' which means it has not desired climate in database
                        print(f"\n>The most desired climate for {species_name} is : NA") #fe1 changed NULL for NA instead, so we are consistent
                    else:
                        ...
                        for climate in cursor:
                            for i in climate:
                                print(">The most likely desired climate for " + species_name + " is : {}".format(i)) #fe1 changed desired
            flag = False
            flag = return_to_menu(flag)
    ...
    # Selection 5 answers question 5 
        ...
        query2 = ('''SELECT classification, avg(average_lifespan) AS al
                     FROM calcu
                     GROUP BY classification ORDER BY al ASC ''') # result will be shown as ASC
        cursor.execute(query2)   
