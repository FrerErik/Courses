...
while(flag):
        ...
        for j in lst1:
            if j[1] != 'NA':  # Filter the species without height information which showing 'NA' in the table
                ...
                if height > average_height:
                    lst2.append(j)
        print("The species with height higher than given number are: ")
        for sp in lst2:
            print(">", sp[0] + ":  " + sp[1])
        flag = False
        flag = return_to_menu(flag)
    ...
    # Selection 4 answers question 4
        ...
        sp_name = input("Please enter the name of the specie for query: ")
        query1 = (
            '''SELECT homeworld FROM species WHERE sp_name= "{}" '''.format(sp_name))
        cursor.execute(query1)
        for i in cursor:
            for j in i:
                if j == 'NA':  # Identify the species without 'homeworld' which means it has not desired climate in database
                    print(">The most likely desired climate of the given species: NULL")
                else:
                    # Query for the species has 'homeworld'
                    query2 = (('''SELECT planets.climate
                                  FROM planets 
                                  JOIN species ON species.homeworld = planets.name
                                  WHERE species.sp_name = "{}" ''').format(sp_name))
                    cursor.execute(query2)
                    for climate in cursor:
                        for i in climate:
                            print(
                                ">The most likely desired climate of the given species: {}".format(i))
        flag = False
        flag = return_to_menu(flag)
    ...
    # Selection 5 answers question 5
        ...
        for i in lst1:
            try:
                cursor.execute(
                    '''INSERT INTO calcu (sp_name, classification, average_lifespan) VALUES (%s,%s,%s)''', (i[0], i[1], int(i[2])))
                cnx.commit()
            except mysql.connector.Error as err:
                cnx.rollback()
